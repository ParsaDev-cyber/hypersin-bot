...
# ═══════════════════════════════════════
import requests, json, time, random, string, os, threading, gc, re, sys, traceback
...
def default_db():
    return {
        ...
        "stats": {
            ...
            "deleted_messages": 0, "total_members": 0,
            "completed_members": 0, "total_transfers": 0,
...
def check_normal_leave(uid, order):
    try:
        ...
        if passed >= 300:
            return
        if str(uid) in order.get("penalized", {}):
            return
        penalty = 3
        back = 2
...
def broadcast_worker(user_id, text):
    ...
    for i, uid in enumerate(all_users, 1):
        ...
        if len(batch) >= BROADCAST_SPEED:
            ...
            time.sleep(1)
            try:
                send_message(user_id, f"📊 پیشرفت: {i:,}/{total:,}\n✅ موفق: {sent:,}\n❌ ناموفق: {failed:,}")
            except:
                pass
    for u in batch:
        try:
    ...
    try:
        ...
        if db.get("pending_support", {}).get(user_id, {}).get("step") == "waiting":
            ...
            try:
                ...
                send_message(chat_id, "❌ خطا در ارسال! دوباره تلاش کن.", main_keyboard(user_id))
            db["pending_support"].pop(user_id, None)
            save_db_async()
            return
# ═══ پایان بخش ۹ ═══
...
# ═══ پایان بخش ۱۲ ═══
        ...
        if text == "🔄 بازیابی سکه":
            ...
            send_message(chat_id,
                ...
                "💰 سقف بازیابی: ۵۰۰ سکه\n"
                "━━━━━━━━━━━━━━━━\n\n"
                "📩 حالا پیام رو فوروارد کن 👇",
                cancel_keyboard()
            )
            db["pending_orders"][user_id] = {"step": "waiting_restore"}
            save_db_async()
            return
        ...
        if pending.get("step") == "waiting_count":
            try:
                ...
                if fwd.get("ok"):
                    ...
                    if rr.get("ok"):
                        db["orders"][oid]["reply_message_id"] = rr["result"]["message_id"]
                    del db["pending_orders"][user_id]
                    save_db_async()
        ...
        if pmem.get("step") == "waiting_admin":
            if text.strip() == "ادمین کردم":
                ...
                try:
                    ...
                    send_message(chat_id, "❌ خطا!", cancel_keyboard())
            else:
...
# ═══ پایان بخش ۱۷ ═══
        if pmem.get("step") == "waiting_count":
            try:
                count = int(convert_number(text))
                if count < MIN_MEMBER:
                    send_message(chat_id, f"❌ حداقل {MIN_MEMBER} عضو!", cancel_keyboard())
                    return
                link = pmem["link"]
                tcid = pmem["chat_id"]
                otype = pmem.get("order_type", "normal")
                cost = get_setting('member_cost', 5) if otype == "normal" else get_setting('member_cost_guaranteed', 10)
                total_cost = count * cost
                if get_coins(user_id) < total_cost:
                    send_message(chat_id, f"❌ سکه کافی نداری!\n\n💰 نیاز: {total_cost:,} ({count} × {cost})", cancel_keyboard())
        ...
        if is_admin(user_id):
            ...
            if pcs.get("key"):
                try:
                    ...
                    send_message(chat_id, "❌ عدد!", settings_keyboard())
                return
            if text == "🚫 مسدود کردن":
                db["pending_ban"][user_id] = {"step": "waiting"}
                save_db_async()
                send_message(chat_id, "🚫 آیدی:", cancel_keyboard())
                return
            if text == "✅ رفع مسدودیت":
            ...
            if text == "👑 افزودن ادمین" and user_id == str(OWNER_ID):
                db["pending_admin"][user_id] = {"step": "waiting"}
                save_db_async()
                send_message(chat_id, "👑 آیدی:", cancel_keyboard())
                return
            if text == "🗑️ حذف ادمین" and user_id == str(OWNER_ID):
                db["pending_remove_admin"][user_id] = {"step": "waiting"}
                save_db_async()
                send_message(chat_id, "🗑️ آیدی:", cancel_keyboard())
                return
            if text == "📨 پیام به کاربر":
                ...
                return
            if text == "💻 اجرای کد":
                db["pending_execute"][user_id] = {"step": "waiting"}
                save_db_async()
                send_message(chat_id, "💻 کد:", cancel_keyboard())
                return
            if text == "🎁 سکه پاکت":
                db["pending_packet"][user_id] = {"step": "waiting_coins"}
                save_db_async()
                send_message(chat_id, "💰 هر نفر چند سکه؟", owner_keyboard())
                return
            ...
            if text.startswith("add ") and is_admin(user_id):
                ...
                if ch not in db.get("join_channels", []):
                    ...
                    save_db_async()
                send_message(chat_id, f"✅ {ch}", owner_keyboard())
                return
            ...
            if text == "🔄 حذف بازیابی" and user_id == str(OWNER_ID):
                ...
                save_db_async()
                send_message(chat_id, "✅ حذف شد!" if current else "✅ فعال شد!", owner_keyboard())
                return
            if text == "🎁 ساخت کد هدیه":
                db["pending_gift"][user_id] = {"step": "waiting_coins"}
                save_db_async()
                send_message(chat_id, "💰 چند سکه؟", owner_keyboard())
                return
            if text == "💰 افزودن سکه به همه":
                db["pending_add_coins"][user_id] = {"step": "waiting"}
                save_db_async()
                send_message(chat_id, "💰 چند سکه؟", owner_keyboard())
                return
            if text == "🎁 تغییر سکه دعوت":
                ...
                return
            if text == "📢 پیام همگانی":
                db["pending_broadcast"][user_id] = {"step": "waiting"}
                save_db_async()
                send_message(chat_id, "📢 متن:", owner_keyboard())
                return
            if text == "🎮 شروع بازی":
                db["pending_utility"][user_id] = {"step": "game_winners"}
                save_db_async()
                send_message(chat_id, "1️⃣ چند نفر برنده؟", owner_keyboard())
                return
            if text == "🔗 کاربردی‌ها":
                db["pending_utility"][user_id] = {"step": "utility_text"}
                save_db_async()
...
# ═══ پایان بخش ۱۹ ═══
            ...
            if pb.get("step") == "waiting":
                ...
                del db["pending_ban"][user_id]
                save_db_async()
                send_message(chat_id, f"🚫 {uid} مسدود شد!", owner_keyboard())
                try:
                    send_message(int(uid), "🚫 مسدود شدی!")
                except:
                    pass
                return
            ...
            if pu2.get("step") == "waiting":
                ...
                if uid in db["banned"]:
                    ...
                    del db["pending_unban"][user_id]
                    save_db_async()
                    send_message(chat_id, f"✅ {uid} آزاد شد!", owner_keyboard())
                    try:
                        send_message(int(uid), "🎉 مسدودی برداشته شد.")
                    except:
                        pass
                else:
                    send_message(chat_id, "❌ نبود!", owner_keyboard())
                return
            ...
            if pa.get("step") == "waiting":
                ...
                del db["pending_admin"][user_id]
                save_db_async()
                send_message(chat_id, "👑 ادمین شد!", owner_keyboard())
                try:
                    send_message(int(uid), "👑 ادمین شدی!", owner_keyboard())
                except:
                    pass
                return
            ...
            if pra.get("step") == "waiting":
                ...
                if uid in db["admins"]:
                    ...
                    try:
                        send_message(int(uid), "🗑️ پنل گرفته شد!", main_keyboard())
                    except:
                        pass
                else:
                    send_message(chat_id, "❌ نبود!", owner_keyboard())
                del db["pending_remove_admin"][user_id]
                save_db_async()
                return
            ppm = db["pending_pm"].get(user_id, {})
            if ppm.get("step") == "waiting_id":
                db["pending_pm"][user_id] = {"step": "waiting_text", "id": text.strip()}
                save_db_async()
                send_message(chat_id, "📝 متن:", cancel_keyboard())
                return
            ...
            if pe.get("step") == "waiting":
                ...
                del db["pending_execute"][user_id]
                save_db_async()
                return
            ...
            if pac.get("step") == "waiting":
                try:
                    ...
                    send_message(chat_id, "❌ عدد!", owner_keyboard())
                return
            ...
            if pv.get("step") == "waiting_id":
                db["pending_vip"][user_id] = {"step": "waiting_seconds", "target": text.strip()}
                save_db_async()
                send_message(chat_id, "⏰ ثانیه:", cancel_keyboard())
                return
            if pv.get("step") == "waiting_seconds":
                try:
                    sec = int(convert_number(text))
                    target = pv["target"]
                    exp = datetime.now() + timedelta(seconds=sec)
                    db["vip_expiry"][target] = str(exp)
                    del db["pending_vip"][user_id]
                    save_db_async()
                    send_message(chat_id, f"⭐ {target} تا {exp.strftime('%Y-%m-%d %H:%M')}", owner_keyboard())
                    try:
                        send_message(int(target), f"⭐ VIP شدی تا {exp.strftime('%Y-%m-%d %H:%M')}")
                    except:
                        pass
                except:
                    send_message(chat_id, "❌ عدد!", cancel_keyboard())
                return
            ...
            if pu.get("step") == "game_winners":
                try:
                    db["pending_utility"][user_id] = {"step": "game_prize", "winners": int(convert_number(text))}
                    save_db_async()
            ...
            if pu.get("step") == "game_prize":
                try:
                    db["pending_utility"][user_id] = {"step": "game_target", "winners": pu["winners"], "prize": int(convert_number(text))}
                    save_db_async()
            ...
            if pu.get("step") == "game_target":
                try:
                    ...
                    del db["pending_utility"][user_id]
                    save_db_async()
            ...
            if pu.get("step") == "utility_text":
                db["pending_utility"][user_id] = {"step": "utility_link", "text": text}
                save_db_async()
                send_message(chat_id, "🔗 لینک:", cancel_keyboard())
                return
            if pu.get("step") == "utility_link":
                ...
                del db["pending_utility"][user_id]
                save_db_async()
                send_message(chat_id, "✅ گذاشته شد!", owner_keyboard())
                return
            ...
            if pp.get("step") == "waiting_capacity":
                try:
                    db["pending_packet"][user_id] = {"step": "waiting_text", "coins": pp["coins"], "capacity": int(convert_number(text))}
                    save_db_async()
                    send_message(chat_id, "📝 متن:", owner_keyboard())
                except:
                    send_message(chat_id, "❌ عدد!", owner_keyboard())
            ...
            if ppu.get("step") == "waiting_id":
                db["pending_packet_user"][user_id] = {"step": "waiting_coins", "id": text.strip()}
                save_db_async()
                send_message(chat_id, "💰 چند سکه؟", cancel_keyboard())
                return
            if ppu.get("step") == "waiting_coins":
                try:
                    ...
                    del db["pending_packet_user"][user_id]
                    save_db_async()
                    send_message(chat_id, f"✅ {amount} به {target}", owner_keyboard())
                    try:
                        send_message(int(target), f"🎁 {amount} سکه هدیه!")
                    except:
                        pass
                except:
                    send_message(chat_id, "❌ عدد!", cancel_keyboard())
...
def handle_callback(callback):
    try:
        ...
        if is_banned(user_id):
            answer_callback(callback_id, "🚫 مسدود شدی!", show_alert=True)
            return
        ...
        if data == "copy_id":
            answer_callback(callback_id, f"✅ {user_id}", show_alert=True)
            return
# ═══ پایان بخش ۲۱ ═══
        if data == "transfer_confirm":
            pt = db["pending_transfer"].get(user_id, {})
            if pt.get("step") != "waiting_confirm":
                answer_callback(callback_id, "❌ منقضی!", show_alert=True)
                return
            target = pt["target"]
            amount = pt["amount"]
            fee = get_setting("transfer_fee", 2)
            recv = pt["receiver_gets"]
            if remove_coins(user_id, amount):
                ...
                try:
                    send_message(int(target), f"🎉 سکه گرفتی!\n\n👤 از: {user_id}\n🪙 {recv:,}\n💰 {get_coins(target):,}")
                except:
                    pass
            else:
                answer_callback(callback_id, "❌ سکه کافی نداری!", show_alert=True)
            del db["pending_transfer"][user_id]
            save_db_async()
            return
        ...
        if data == "transfer_cancel":
            ...
            send_message(user_id, "❌ لغو شد!", main_keyboard(user_id))
            return
        ...
        if data.startswith("packet_"):
            ...
            if str(user_id) in p["used_by"]:
                answer_callback(callback_id, "⚠️ قبلاً!", show_alert=True)
                return
            ...
            save_db_async()
            answer_callback(callback_id, f"🎉 {p['coins']:,} سکه!", show_alert=True)
            return
# ═══ پایان بخش ۲۲ ═══
        # ═══ دیدم (سین) ═══
        if data.startswith("seen_"):
            ...
            if order["status"] != "active":
                answer_callback(callback_id, "✅ تموم!")
                return
            if str(user_id) in db["seen_records"].get(oid, []):
                answer_callback(callback_id, "⚠️ قبلاً!")
                return
        ...
        if data.startswith("report_"):
            ...
            try:
                send_message(int(OWNER_ID), f"🚨 گزارش سین\n\n👤 @{rn}\n📝 #{order.get('order_number', '?')}\n🔢 {order['count']}\n👁️ {order['seen_count']}")
            except:
                pass
            return
...
# ═══ پایان بخش ۲۳ ═══
        ...
        if data.startswith("mjoin_"):
            ...
            if order["status"] != "active":
                answer_callback(callback_id, "✅ تموم!", show_alert=True)
                return
            if str(user_id) in db["member_records"].get(mid, []):
                answer_callback(callback_id, "⚠️ قبلاً!", show_alert=True)
                return
            if order.get("user_id") == str(user_id):
                answer_callback(callback_id, "❌ تو سفارش خودت!", show_alert=True)
                return
            tcid = order["chat_id"]
            ...
            if ms.get("ok") and ms["result"]["status"] in ["member", "administrator", "creator"]:
                ...
                if order.get("message_id"):
                    ...
                    try:
                        edit_message_text(CHANNEL_ID, order["message_id"], f"📋 سفارش عضو - {tname}\n\n🔗 لینک کانال: {order['link']}\n👥 تعداد درخواستی: {count}\n✅ تعداد عضو شده: {ns}\n#{mnum}\n\n🪙 {reward} سکه میگیری!", kb)
                    except:
                        pass
                if ns >= count:
                    order["status"] = "completed"
